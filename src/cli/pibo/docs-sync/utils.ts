...
export function sshCheck(host: string, user = "root", keyPath?: string): boolean {
  ...
  const result = run(`ssh ${keyOpt} -o ConnectTimeout=5 -o BatchMode=yes -o StrictHostKeyChecking=no ${user}@${host} "echo ok" 2>/dev/null`);
  return result === "ok";
}

/** Check git repo at path */
export function gitRepoCheck(path: string): boolean {
  return existsSync(`${path}/.git`) || run(`git -C ${path} rev-parse --git-dir 2>/dev/null`) !== null;
}
