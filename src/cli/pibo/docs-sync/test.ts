...
function ssh(sshKey: string, serverIp: string, cmd: string) {
  return run(`ssh -i ${sshKey} -o StrictHostKeyChecking=no -o ConnectTimeout=5 root@${serverIp} ${JSON.stringify(cmd)} 2>/dev/null`);
}
...
export async function runTest() {
  ...
  run(`cd ${docsPath} && git add -A && (git commit -m "test: cleanup sync-test" >/dev/null 2>&1 || true)`);
  await sleep(2000);
  ...
  console.log(createdRemote ? ok("Lokale Datei kam auf dem Server an") : fail("Lokale Datei kam NICHT auf dem Server an"));
  ...
  const movedRemote = await waitUntil(
    () => remoteExists(sshKey, serverIp, serverDocs, MOVED_FILE) && !remoteExists(sshKey, serverIp, serverDocs, LOCAL_FILE),
    45000,
    1000
  );
  console.log(movedRemote ? ok("Move wurde korrekt als Move synchronisiert") : fail("Move blieb auf dem Server als Copy/Altdatei hängen"));
  if (!movedRemote) return;
  ...
  console.log(remoteToLocal ? ok("Server-Änderung wurde lokal schnell übernommen") : fail("Server-Änderung kam nicht zeitnah lokal an"));
  if (!remoteToLocal) return;
  ...
  const serverMoveLocal = await waitUntil(
    () => run(`test -f ${join(docsPath, TEST_ROOT, "server-moved/from-server.md")} && echo yes || echo no`) === "yes"
      && run(`test -f ${join(docsPath, REMOTE_FILE)} && echo yes || echo no`) === "no",
    20000,
    1000
  );
  console.log(serverMoveLocal ? ok("Server-Move wurde lokal korrekt gespiegelt") : fail("Server-Move blieb lokal als Copy/Altdatei hängen"));
  if (!serverMoveLocal) return;
  ...
  console.log(localDeleteRemote ? ok("Lokales Löschen wurde auf dem Server übernommen") : fail("Lokales Löschen wurde auf dem Server NICHT übernommen"));
  if (!localDeleteRemote) return;

  ssh(sshKey, serverIp, `rm -f ${serverDocs}/${TEST_ROOT}/server-moved/from-server.md`);
  const remoteDeleteLocal = await waitUntil(
    () => run(`test -f ${join(docsPath, TEST_ROOT, "server-moved/from-server.md")} && echo yes || echo no`) === "no",
    20000,
    1000
  );
  console.log(remoteDeleteLocal ? ok("Server-Löschen wurde lokal übernommen") : fail("Server-Löschen wurde lokal NICHT übernommen"));

  // Cleanup
  rmSync(join(docsPath, TEST_ROOT), { recursive: true, force: true });
  ssh(sshKey, serverIp, `rm -rf ${serverDocs}/${TEST_ROOT}`);

  console.log("\n" + (remoteDeleteLocal ? bold("✅ Docs-Sync-Test erfolgreich") : bold("⚠️ Docs-Sync-Test mit Fehlern beendet")));
}
