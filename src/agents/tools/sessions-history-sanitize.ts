...
export const SESSIONS_HISTORY_TEXT_MAX_CHARS = 4000;
...
export function sanitizeSessionHistoryMessage(message: unknown): {
  message: unknown;
  truncated: boolean;
  redacted: boolean;
} {
...
export function sanitizeSessionHistoryMessages(messages: unknown[]): {
  messages: unknown[];
  contentTruncated: boolean;
  contentRedacted: boolean;
} {
  const sanitized = messages.map((message) => sanitizeSessionHistoryMessage(message));
  return {
    messages: sanitized.map((entry) => entry.message),
    contentTruncated: sanitized.some((entry) => entry.truncated),
...
export function capSessionHistoryMessages(params: { messages: unknown[]; maxBytes?: number }): {
  messages: unknown[];
  bytes: number;
  truncated: boolean;
  ...
  const capped = capArrayByJsonBytes(params.messages, maxBytes);
  const droppedMessages = capped.items.length < params.messages.length;
  ...
  return {
    ...
    truncated: droppedMessages || hardened.hardCapped,
