...
export function App() {
  const initialSettings = useMemo(() => loadCustomUiSettings(), []);
  ...
  const [chatSnapshot, setChatSnapshot] = useState<ChatSnapshot>(() =>
    snapshotHost(hostRef.current),
  );
  ...
  useEffect(() => {
    void loadBootstrapConfig().then((nextBootstrap) => {
      ...
      setSettings((current) =>
        current.gatewayUrl === defaultGatewayUrl
  ...
  useEffect(() => {
    ...
    const client = new GatewayBrowserClient({
      ...
      onHello: (nextHello) => {
        ...
        const snapshot = nextHello.snapshot as
        ...
        host.client = client;
        host.lastError = null;
        host.sessionKey = resolveSessionKeyWithDefaults(
          host.sessionKey,
  ...
  const renderableMessages = useMemo(() => {
    ...
    chatSnapshot.chatStream,
    chatSnapshot.chatStreamSegments,
    chatSnapshot.chatToolMessages,
  ]);
            ...
            >
              <div className="rounded-[24px] border border-cyan-400/20 bg-cyan-400/8 p-4">
                ...
                <div className="grid gap-2 text-sm text-slate-300">
                  ...
                  <p className="text-xs leading-5 text-slate-400">
                    Mounted as a standalone Control-UI-compatible app and ready for
              ...
              <div className="min-h-0 flex-1 rounded-[24px] border border-white/8 bg-black/10 p-3">
                <div className="mb-3 flex items-center justify-between px-1">
                  ...
                  >
                    <LoaderCircleIcon
                      className={cn("size-4", chatSnapshot.chatLoading && "animate-spin")}
                    />
                  </Button>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="truncate text-xs text-slate-400">
                          {currentSessionSubtitle(session)}
                        </span>
            ...
            <section className="flex min-h-[80svh] min-w-0 flex-col gap-4 rounded-[28px] border border-white/8 bg-slate-950/60 p-3 md:p-4 lg:min-h-[75vh]">
              ...
              <header className="grid gap-4 rounded-[24px] border border-white/8 bg-white/[0.03] p-4 lg:grid-cols-[1fr_auto]">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 truncate text-sm text-slate-400">
                    {currentSessionSubtitle(currentSession)}
                  </p>
                </div>
                  ...
                  <ConversationContent className="p-5">
                    {deferredMessages.length === 0 ? (
                      ...
                      deferredMessages.map((message) => (
                        <MessageCard
                          assistantAvatar={bootstrap.assistantAvatar}
                          key={message.id}
                          message={message}
                        />
                      ))
              ...
              >
                ...
                <PromptInputFooter className="mt-3 items-center justify-between gap-3">
                  ...
                  <PromptInputSubmit onStop={() => void handleAbortRun()} status={chatStatus} />
                </PromptInputFooter>
              </PromptInput>
            </section>
          </section>
