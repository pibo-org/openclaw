...
const DEV_GATEWAY_PROXY_PATH = "/__openclaw/gateway";
...
export function buildDefaultGatewayUrl(basePath: string): string {
  ...
  if (import.meta.env.DEV) {
    return `${protocol}//${window.location.host}${DEV_GATEWAY_PROXY_PATH}`;
  }
...
export async function loadModelCatalog(client: {
...
export async function filePartToChatAttachment(
  file: FileUIPart,
  index: number,
  ...
  try {
    ...
    const dataUrl = await new Promise<string>((resolve, reject) => {
      ...
      reader.addEventListener("error", () => reject(new Error("Failed to read attachment.")), {
        once: true,
      });
...
export async function filePartsToChatAttachments(files: FileUIPart[]) {
  const items = await Promise.all(
    files.map((file, index) => filePartToChatAttachment(file, index)),
  );
