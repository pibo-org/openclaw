...
export function createPluginRuntimeMock(overrides: DeepPartial<PluginRuntime> = {}): PluginRuntime {
  ...
  const base: PluginRuntime = {
    ...
    piboWorkflows: {
      list: vi.fn(async () => []),
      describe: vi.fn(async (moduleId: string) => ({
        moduleId,
        displayName: `Workflow ${moduleId}`,
        description: "Test workflow",
        kind: "agent_workflow" as const,
        version: "1.0.0-test",
        requiredAgents: [],
        terminalStates: ["done", "failed"] as Array<
          "done" | "blocked" | "aborted" | "failed" | "max_rounds_reached"
        >,
        supportsAbort: true,
        inputSchemaSummary: [],
        artifactContract: [],
      })),
      start: vi.fn(async (moduleId: string) => ({
      ...
      startAsync: vi.fn(async (moduleId: string) => ({
        runId: "workflow-run-async-1",
        moduleId,
        status: "running" as const,
        terminalReason: null,
        abortRequested: false,
      ...
      progress: vi.fn(async (runId: string) => ({
        runId,
        moduleId: "test-workflow",
        status: "running" as const,
        ...
        sessions: {},
        humanSummary: "Run is active.",
      })),
      abort: vi.fn(async (runId: string) => ({
        runId,
        moduleId: "test-workflow",
      ...
      runs: vi.fn(async () => []),
      traceSummary: vi.fn(async (runId: string) => ({
        runId,
        moduleId: "test-workflow",
        traceLevel: 1 as const,
        status: "running" as const,
      ...
      traceEvents: vi.fn(async () => []),
      artifacts: vi.fn(async () => []),
      readArtifact: vi.fn(async (runId: string, name: string) => ({
        name,
        path: `/tmp/${runId}/${name}`,
        sizeBytes: 0,
        updatedAt: new Date().toISOString(),
