...
function isWindowPlacementPrefKey(key: string): boolean {
  return key === "window_placement" || key.startsWith("window_placement_");
}

function browserThemeColor(browser: Record<string, unknown>): number | null {
  const theme = isRecord(browser.theme) ? browser.theme : null;
  return typeof theme?.user_color2 === "number" ? theme.user_color2 : null;
}
...
export function healOpenClawProfileLaunchPrefs(userDataDir: string): {
  removedBrowserKeys: string[];
  removedBrowserBlock: boolean;
} {
  const preferencesPath = path.join(userDataDir, "Default", "Preferences");
  const prefs = safeReadJson(preferencesPath);
  if (!prefs) {
    return { removedBrowserKeys: [], removedBrowserBlock: false };
  }
  const browser = isRecord(prefs.browser) ? { ...prefs.browser } : null;
  if (!browser) {
    return { removedBrowserKeys: [], removedBrowserBlock: false };
  }

  const removedBrowserKeys = Object.keys(browser).filter((key) => isWindowPlacementPrefKey(key));
  if (removedBrowserKeys.length === 0) {
    return { removedBrowserKeys: [], removedBrowserBlock: false };
  }

  const themeColor = browserThemeColor(browser);
  if (
    themeColor != null &&
    (!isRecord(prefs.autogenerated) ||
      !isRecord(prefs.autogenerated.theme) ||
      typeof (prefs.autogenerated.theme as Record<string, unknown>).color !== "number")
  ) {
    setDeep(prefs, ["autogenerated", "theme", "color"], themeColor);
  }
  delete prefs.browser;
  safeWriteJson(preferencesPath, prefs);
  return { removedBrowserKeys, removedBrowserBlock: true };
}
