...
describe("browser chrome profile decoration", () => {
  ...
  it("heals stale browser window placement prefs while preserving theme", async () => {
    const userDataDir = await createUserDataDir();
    await fsp.mkdir(path.join(userDataDir, "Default"), { recursive: true });
    await fsp.writeFile(
      path.join(userDataDir, "Default", "Preferences"),
    ...
    const result = healOpenClawProfileLaunchPrefs(userDataDir);

    expect(result.removedBrowserKeys.toSorted()).toEqual([
      "window_placement",
      "window_placement_popup",
    ]);
    expect(result.removedBrowserBlock).toBe(true);

    const prefs = await readJson(path.join(userDataDir, "Default", "Preferences"));
    expect(prefs.browser).toBeUndefined();
    expect(prefs.autogenerated).toEqual({
      theme: {
        color: -47872,
      },
    });
  ...
  it("heals zero-work-area browser window placement prefs too", async () => {
    const userDataDir = await createUserDataDir();
    await fsp.mkdir(path.join(userDataDir, "Default"), { recursive: true });
    await fsp.writeFile(
      path.join(userDataDir, "Default", "Preferences"),
    ...
    expect(result.removedBrowserKeys).toEqual(["window_placement"]);
    expect(result.removedBrowserBlock).toBe(true);
  ...
  it("does not recreate browser prefs during decoration", async () => {
    const userDataDir = await createUserDataDir();
