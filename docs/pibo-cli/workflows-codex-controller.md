...
3. If the wrapper default is unset, `fastMode` still defaults to `true`; model and reasoning effort fall back to the Codex SDK / Codex defaults.
...
`codex_controller` starts Codex workers in fast mode by default. OpenClaw `thinkingDefault`, Codex `model_reasoning_effort`, wrapper `effort`, and wrapper `fastMode` are separate settings. A model can support `xhigh` while the operating default remains `high`.
